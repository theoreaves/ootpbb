    ...
    <div class="relative mb-4">
        ...
        <div class="bg-[{{ $league->background_color_id }}] text-[{{ $league->text_color_id }}] flex flex-1 items-center justify-between pl-36 ">
            ...
            <div>
                ...
                <div>Home | <a href="{{ route('home.standings') }}">Standings</a> | Teams</div>
            </div>
            <div></div>
        </div>
    </div>
        ...
        <div class="w-full md:w-1/3 lg:w-1/4 xl:w-1/6 bg-gray-100 rounded shadow p-4">
            ...
            <div class="flex justify-between items-center border-b pb-1 mb-1">
                ...
                <div class="text-right flex gap-2">
                    <span class="w-5">{{ $game->runs0 ?? '-' }}</span>
                    <span class="w-5">{{ $game->hits0 ?? '-' }}</span>
                    <span class="w-5">{{ $game->errors0 ?? '-' }}</span>
                </div>
            </div>
            <div class="flex justify-between items-center">
                ...
                <div class="text-right flex gap-2">
                    <span class="w-5">{{ $game->runs1 ?? '-' }}</span>
                    <span class="w-5">{{ $game->hits1 ?? '-' }}</span>
                    <span class="w-5">{{ $game->errors1 ?? '-' }}</span>
                </div>
            </div>
        </div>
    @endforeach
